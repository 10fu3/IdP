...
public class URLLogin {
    ...
    public static void mainFlow(io.javalin.http.Context ctx){
        ...
        if(loginResult == LoginResult.SUCCESS){
           ILoginTokenStore.getInstance().putToken(token,loginResult.account.getUUID());

           //ログイン時刻を更新する
           IAccountStore.getInstance().updateAccountInSQL(AccountBuilder.Edit(loginResult.account).setLastLogin(System.currentTimeMillis()/1000L).build());
